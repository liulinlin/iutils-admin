							...
							<div class="am-u-sm-12 am-u-md-3 am-u-lg-3">
								<div class="am-btn-toolbar">
									<div class="am-btn-group am-btn-group-xs">
										<shiro:hasPermission name="sys:slog:edit"><!-- 设置是否有删除权限标注 --><c:set var="hasDel" value="true"></c:set>
											<button type="button" id="alldelete" class="am-btn am-btn-default am-btn-danger"><span class="am-icon-trash-o"></span> 删除
											...
											<c:if test="${not empty page.entity.username}"><span class="tags"><input type="hidden" name="username" value="${page.entity.username}" />用户=${page.entity.username} <a href="javascript:;" onclick="$(this).parent().remove()">x</a></span></c:if>
                                                <span class="am-select am-input-group-sm">
                                                     <input type="text" class="am-select-input" autocomplete="off" style="border: none;"
															placeholder="关键字" am-data='[{"field":"username","desc":"用户","type":"string"}]'>
									...
									<table id="contentTable" class="am-table am-table-compact am-table-striped tpl-table-black">
										...
										<c:forEach items="${page.list}" var="slog" varStatus="status">
											<tr>
												...
												<td><a href="javascript:;" onclick="openModel(false,'${ctx}/slog/update?id=${slog.id}')" title="查看详细"><span class="am-icon-eye"></span></a></td>
											</tr>
										</c:forEach>
										</tbody>
									</table>
